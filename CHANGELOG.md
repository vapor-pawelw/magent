...
### Sidebar
- PR and Jira ticket status badges now appear as small colored pills inline with the ticket/PR labels in the sidebar (e.g. "Open" in green, "In Progress" in blue, "Merged" in purple). PR badges also detect draft status. Jira badge colors are sourced from the Jira API status category. Both can be toggled independently in Settings > Threads > Sidebar.
- A "Synced Xm ago" label at the top of the sidebar shows when PR and Jira statuses were last refreshed. A refresh button triggers an immediate re-sync.
