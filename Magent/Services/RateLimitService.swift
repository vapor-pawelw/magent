...
/// Owns all rate-limit detection, parsing, caching, and notification logic.
/// Extracted from `ThreadManager+RateLimit`.
///
/// Thread/delegate notifications remain in `ThreadManager` — this service mutates
/// `store.threads` directly and returns changed thread IDs for the caller to fan out.
final class RateLimitService {
    ...
    /// Resolves the effective agent type for a given thread + session.
    /// Injected by ThreadManager so we don't pull in helpers that live there.
    var resolveAgentType: ((_ thread: MagentThread, _ sessionName: String) -> AgentType?)?

    // MARK: - Rate-Limit State (moved from ThreadManager)

    var globalAgentRateLimits: [AgentType: AgentRateLimitInfo] = [:]
    /// Persisted cache of seen rate-limit fingerprints → concrete resetAt entries.
    /// Time-only entries are kept as tombstones after expiry to prevent stale pane text
    /// like "resets 11am" from being re-anchored daily.
    ...
    init(store: ThreadStore, persistence: PersistenceService, tmux: TmuxService) {
        self.store = store
        self.persistence = persistence
        self.tmux = tmux
    }

    // MARK: - Agent Type Helper

    private func agentType(for thread: MagentThread, sessionName: String) -> AgentType? {
        resolveAgentType?(thread, sessionName)
    }
    ...
    func checkForRateLimitedSessions() async {
        ...
        if !detectionEnabled {
            ...
            for i in store.threads.indices {
                let promptOnly = store.threads[i].rateLimitedSessions.filter { $0.value.isPromptBased }
                if store.threads[i].rateLimitedSessions.count != promptOnly.count {
                    store.threads[i].rateLimitedSessions = promptOnly
                    changed = true
                }
            }
        ...
        let activeSession: String? = store.threads.first(where: { $0.id == store.activeThreadId })?.lastSelectedTabIdentifier
        let rateLimitThrottle: TimeInterval = 15

        let rateLimitSnapshot = store.threads.filter { !$0.isArchived }
        for thread in rateLimitSnapshot {
            ...
            if let j = store.threads.firstIndex(where: { $0.id == threadId }),
               updatedRateLimits != store.threads[j].rateLimitedSessions {
                // Mark newly rate-limited sessions as unread for "requires attention" UX.
                let previousSessions = Set(store.threads[j].rateLimitedSessions.keys)
                let newSessions = Set(updatedRateLimits.keys).subtracting(previousSessions)
                if !newSessions.isEmpty {
                    store.threads[j].unreadRateLimitSessions.formUnion(newSessions)
                }
                store.threads[j].rateLimitedSessions = updatedRateLimits
                changedThreadIds.insert(threadId)
            }
        }
    ...
    func applyRateLimitMarker(
        ...
        for i in store.threads.indices where !store.threads[i].isArchived {
            var updatedRateLimits = store.threads[i].rateLimitedSessions

            for sessionName in store.threads[i].agentTmuxSessions {
                guard agentType(for: store.threads[i], sessionName: sessionName) == agent else { continue }
            ...
            if updatedRateLimits != store.threads[i].rateLimitedSessions {
                store.threads[i].rateLimitedSessions = updatedRateLimits
    ...
    func clearPromptRateLimitMarkers(for agent: AgentType, changedThreadIds: inout Set<UUID>) -> Bool {
        ...
        for i in store.threads.indices where !store.threads[i].isArchived {
            ...
            let promptKeys = updatedRateLimits.keys.filter { sessionName in
                ...
                return agentType(for: store.threads[i], sessionName: sessionName) == agent
            }
    ...
    private func clearRateLimitMarkers(
        ...
        var resumeMarkedThreadIds = Set<UUID>()
        for i in store.threads.indices {
            var filtered = store.threads[i].rateLimitedSessions
            let keysToRemove = filtered.keys.filter { sessionName in
                agentType(for: store.threads[i], sessionName: sessionName) == agent
            }
            ...
            store.threads[i].rateLimitedSessions = filtered
            store.threads[i].unreadRateLimitSessions.subtract(keysToRemove)
            changedThreadIds.insert(store.threads[i].id)
            ...
            if markResumeNeeded {
                ...
                let toMark = keysToRemove.filter {
                    ...
                    !store.threads[i].busySessions.contains($0) &&
                    !store.threads[i].waitingForInputSessions.contains($0)
                }
                if !toMark.isEmpty {
                    store.threads[i].waitingForInputSessions.formUnion(toMark)
                    onRateLimitLiftResumeNeeded?(Set(toMark))
                    resumeMarkedThreadIds.insert(store.threads[i].id)
                }
            }
        }
    ...
    /// Injected by ThreadManager to handle per-session resume state when a rate-limit
    /// expires. Called with the set of sessions that need "waiting for resume" markers.
    var onRateLimitLiftResumeNeeded: ((Set<String>) -> Void)?
    ...
    // MARK: - Agent detection helpers (pure utilities, duplicated from ThreadManager+Helpers)
