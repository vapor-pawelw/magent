...
/// Owns sidebar display ordering, section assignment, and dock badge logic.
/// Extracted from `ThreadManager+SectionOrdering`.
///
...
final class SidebarOrderingService {
    ...
    func placeThreadAtBottomOfSidebarGroup(
        ...
        .map(\.displayOrder)
        .max() ?? -1
        store.threads[index].displayOrder = maxOrder + 1
    }
    ...
    func placeThreadAfterSibling(threadId: UUID, afterThreadId: UUID) {
        guard let threadIndex = store.threads.firstIndex(where: { $0.id == threadId }),
              let siblingIndex = store.threads.firstIndex(where: { $0.id == afterThreadId }) else {
            placeThreadAtBottomOfSidebarGroup(threadId: threadId)
            return
        }

        let settings = persistence.loadSettings()
        let sibling = store.threads[siblingIndex]
        ...
        for peer in group where peer.displayOrder > siblingOrder {
            if let i = store.threads.firstIndex(where: { $0.id == peer.id }) {
                store.threads[i].displayOrder += 1
            }
        }
        store.threads[threadIndex].displayOrder = siblingOrder + 1
    }
    ...
    func updateDockBadge() {
        ...
        let unreadCount = store.threads.filter { !$0.isArchived && $0.hasUnreadAgentCompletion }.count
    ...
    func moveThread(_ thread: MagentThread, toSection sectionId: UUID) {
        guard let index = store.threads.firstIndex(where: { $0.id == thread.id }) else { return }
        store.threads[index].sectionId = sectionId
    ...
    func toggleThreadPin(threadId: UUID) {
        guard let index = store.threads.firstIndex(where: { $0.id == threadId }) else { return }
        let wasPinned = store.threads[index].isPinned
        store.threads[index].isPinned.toggle()
        if store.threads[index].isPinned {
            store.threads[index].isSidebarHidden = false
        }
        if wasPinned {
    ...
    var favoriteThreadCount: Int {
        store.threads.filter { !$0.isArchived && $0.isFavorite }.count
    }
    ...
    func canAddFavoriteThread(excludingThreadId: UUID? = nil) -> Bool {
        let count = store.threads.filter {
            !$0.isArchived && $0.isFavorite && $0.id != excludingThreadId
        }.count
        return count < ThreadManager.maxFavoriteThreadCount
    }
    ...
    func toggleThreadFavorite(threadId: UUID) -> Bool {
        ...
        if store.threads[index].isFavorite {
            store.threads[index].isFavorite = false
            store.threads[index].favoritedAt = nil
        } else {
            guard canAddFavoriteThread(excludingThreadId: threadId) else {
                return false
            }
            store.threads[index].isFavorite = true
            store.threads[index].favoritedAt = Date()
        }
    ...
    func toggleThreadHidden(threadId: UUID) {
        ...
        if store.threads[index].isSidebarHidden {
            store.threads[index].isPinned = false
        }
        placeThreadAtBottomOfSidebarGroup(threadId: threadId)
    ...
    func reorderThread(_ threadId: UUID, toIndex targetIndex: Int, inSection sectionId: UUID?) {
        ...
        for group in ThreadSidebarListState.allCases {
            ...
            for (order, groupedThread) in threadsInGroup.enumerated() {
                if let i = store.threads.firstIndex(where: { $0.id == groupedThread.id }) {
                    store.threads[i].displayOrder = order
                }
            }
        }
    ...
    func bumpThreadToTopOfSection(_ threadId: UUID) {
        ...
        store.threads[index].displayOrder = groupMin - 1
    }
    ...
    func reassignThreadsAssigned(
        ...
        var movedCount = 0
        for index in store.threads.indices where threadUsesSection(store.threads[index], sectionId: oldSectionId, projectId: projectId, settings: settings) {
            store.threads[index].sectionId = newSectionId
            movedCount += 1
        }
    ...
    func reassignThreads(fromSection oldSectionId: UUID, toSection newSectionId: UUID) {
        var changed = false
        for i in store.threads.indices where store.threads[i].sectionId == oldSectionId {
            store.threads[i].sectionId = newSectionId
            changed = true
        }
