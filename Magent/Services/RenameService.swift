...
final class RenameService {
    ...
    /// Returns the effective agent type for a project (wired to ThreadManager.effectiveAgentType(for:)).
    var effectiveAgentType: ((UUID) -> AgentType?)?
    /// Returns whether a given agent type currently has an active rate limit.
    var detectedAgentTypeInSession: ((String) async -> AgentType?)?
    /// Called after a successful branch rename so Jira ticket detection refreshes for the thread.
    var verifyDetectedJiraTickets: ((Set<UUID>) async -> Void)?
    /// Provides branch-symlink creation after rename (wired to ThreadManager.ensureBranchSymlink).
    ...
    /// Set of thread IDs currently undergoing an AI-driven slug-generation rename.
    ...
    private func slugGenerationAgentOrder(preferred preferredAgent: AgentType?, projectId: UUID?) -> (allTrackable: [AgentType], available: [AgentType]) {
        ...
        let resolvedPreferred: AgentType? = {
            ...
            if let projectId {
                return effectiveAgentType?(projectId)
            }
    ...
    private func applyGeneratedRenameMetadataIfNeeded(
        ...
        var didApply = false
        if applyDescription {
            let shouldPrefixDraft = prefixDraft || store.threads[currentIndex].hasDraftTabs
        ...
        if applyIcon {
            ...
            if settings.autoSetThreadIconFromWorkType,
               !store.threads[currentIndex].isThreadIconManuallySet {
                store.threads[currentIndex].threadIcon = generatedTaskDescription.suggestedIcon
                didApply = true
            }
        }
    ...
    func renameThread(
        ...
        await verifyDetectedJiraTickets?([thread.id])
    }
    ...
    private func retargetBaseBranches(oldBranch: String, newBranch: String, projectId: UUID) {
        ...
        for i in store.threads.indices where store.threads[i].projectId == projectId {
            // Retarget thread.baseBranch (set at creation time).
            if let base = store.threads[i].baseBranch, base == oldBranch {
                store.threads[i].baseBranch = newBranch
            }

            // Retarget worktree cache override (set via context menu, CLI, or PR target).
            let key = store.threads[i].worktreeKey
    ...
    private func performAutoRename(
        ...
        guard let refreshedIndex = store.threads.firstIndex(where: { $0.id == threadId }) else { return false }
        let refreshedThread = store.threads[refreshedIndex]
        ...
        let preferredAgent = effectiveAgentType?(refreshedThread.projectId)
    ...
    func stripDraftDescriptionPrefixIfNeeded(threadId: UUID) {
        ...
        guard let desc = store.threads[index].taskDescription,
              desc.hasPrefix(Self.draftDescriptionPrefix) else { return }
        // Only strip if the thread no longer has draft tabs.
        guard !store.threads[index].hasDraftTabs else { return }
        store.threads[index].taskDescription = String(desc.dropFirst(Self.draftDescriptionPrefix.count))
        try? persistence.saveActiveThreads(store.threads)
        Task { @MainActor in
    ...
    private func markFirstPromptAutoRenameHandled(threadId: UUID) {
        guard let index = store.threads.firstIndex(where: { $0.id == threadId }) else { return }
        guard !store.threads[index].didAutoRenameFromFirstPrompt else { return }
        store.threads[index].didAutoRenameFromFirstPrompt = true
        try? persistence.saveActiveThreads(store.threads)
    }
    ...
    func sanitizeGeneratedTaskDescription(_ raw: String) -> GeneratedTaskDescription? {
    ...
    private func generateAndPersistTaskDescription(
        ...
        guard let index = store.threads.firstIndex(where: { $0.id == threadId }) else { return nil }
        let thread = store.threads[index]
        ...
        let agentOrder = slugGenerationAgentOrder(preferred: effectiveAgentType?(thread.projectId), projectId: thread.projectId)
    ...
    func generateTaskDescriptionIfNeeded(threadId: UUID, prompt: String) async {
        ...
        if store.threads.first(where: { $0.id == threadId })?.syncWithJira == true { return }
    ...
    func setThreadIcon(threadId: UUID, icon: ThreadIcon, markAsManualOverride: Bool = true) throws {
        ...
        let shouldUpdateIcon = store.threads[index].threadIcon != icon
        let shouldUpdateManualFlag = markAsManualOverride && !store.threads[index].isThreadIconManuallySet
        guard shouldUpdateIcon || shouldUpdateManualFlag else { return }

        if shouldUpdateIcon {
            store.threads[index].threadIcon = icon
        }
        if markAsManualOverride {
            store.threads[index].isThreadIconManuallySet = true
        }
        try persistence.saveActiveThreads(store.threads)
        onThreadsChanged?()
    }
    ...
    func setThreadSignEmoji(threadId: UUID, signEmoji: String?) throws {
        ...
        guard store.threads[index].signEmoji != signEmoji else { return }
        store.threads[index].signEmoji = signEmoji
        try persistence.saveActiveThreads(store.threads)
        onThreadsChanged?()
    }
    ...
    func setThreadPriority(threadId: UUID, priority: Int?) throws {
        ...
        guard store.threads[index].priority != clamped else { return }
        store.threads[index].priority = clamped
        try persistence.saveActiveThreads(store.threads)
        onThreadsChanged?()
    }
    ...
    func setSyncWithJira(threadId: UUID, enabled: Bool) throws {
        ...
        guard store.threads[index].syncWithJira != enabled else { return }
        store.threads[index].syncWithJira = enabled
        ...
        if enabled {
            ...
            let id = threadId
            Task { [weak self] in
                await self?.verifyDetectedJiraTickets?([id])
            }
        }
    }
    ...
    func renameTab(
        ...
        remapTransientSessionState: (Int, [String: String]) -> Bool,
        remapInitialPromptInjectionState: ([String: String]) -> Bool,
        ...
        if currentThread.pinnedTmuxSessions.contains(sessionName) {
            store.threads[index].pinnedTmuxSessions = currentThread.pinnedTmuxSessions.map {
                $0 == sessionName ? resolvedSessionName : $0
            }
        }
        if currentThread.protectedTmuxSessions.contains(sessionName) {
            store.threads[index].protectedTmuxSessions.remove(sessionName)
            store.threads[index].protectedTmuxSessions.insert(resolvedSessionName)
        }
        ...
        store.threads[index].manuallyRenamedTabs.remove(sessionName)
        store.threads[index].manuallyRenamedTabs.insert(resolvedSessionName)
        if currentThread.lastSelectedTabIdentifier == sessionName {
            store.threads[index].lastSelectedTabIdentifier = resolvedSessionName
        }
        if currentThread.unreadCompletionSessions.contains(sessionName) {
            ...
            store.threads[index].unreadCompletionSessions.insert(resolvedSessionName)
        }
        if let info = currentThread.rateLimitedSessions[sessionName] {
            store.threads[index].rateLimitedSessions.removeValue(forKey: sessionName)
            store.threads[index].rateLimitedSessions[resolvedSessionName] = info
        }
        if let conversationID = currentThread.sessionConversationIDs[sessionName] {
            store.threads[index].sessionConversationIDs.removeValue(forKey: sessionName)
            store.threads[index].sessionConversationIDs[resolvedSessionName] = conversationID
        }
        if let sessionAgentType = currentThread.sessionAgentTypes[sessionName] {
            store.threads[index].sessionAgentTypes.removeValue(forKey: sessionName)
            store.threads[index].sessionAgentTypes[resolvedSessionName] = sessionAgentType
        }
        if let sessionCreatedAt = currentThread.sessionCreatedAts[sessionName] {
            store.threads[index].sessionCreatedAts.removeValue(forKey: sessionName)
            store.threads[index].sessionCreatedAts[resolvedSessionName] = sessionCreatedAt
        }
        if currentThread.freshAgentSessions.contains(sessionName) {
            store.threads[index].freshAgentSessions.remove(sessionName)
            store.threads[index].freshAgentSessions.insert(resolvedSessionName)
        }
        if currentThread.forwardedTmuxSessions.contains(sessionName) {
            store.threads[index].forwardedTmuxSessions.remove(sessionName)
            store.threads[index].forwardedTmuxSessions.insert(resolvedSessionName)
        }
        ...
        reKeyKnownGoodSessionContext(sessionName, resolvedSessionName)

        // Update custom tab names: remove old key, store under new key
        store.threads[index].customTabNames.removeValue(forKey: sessionName)
        store.threads[index].customTabNames[resolvedSessionName] = resolvedDisplayName
        ...
        if store.threads[index].agentTmuxSessions.contains(resolvedSessionName) {
            await forceSetupBellPipe(resolvedSessionName)
        }
