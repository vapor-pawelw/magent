...
- `listTabs` populates per-tab status fields (`isBusy`, `isWaitingForInput`, `hasUnreadCompletion`, `isBlockedByRateLimit`, `agentType`) on each `IPCTabInfo` from the thread's per-session sets.
