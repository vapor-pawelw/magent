...
- For Homebrew installs, the original detached-script flow is kept: after the app exits, the updater runs `brew update --quiet` (to refresh the tap so the newest cask is visible), then `brew upgrade --cask magent`, then clears launch-blocking xattrs from `/Applications/Magent.app` before reopening it.
...
- The Homebrew updater script MUST run `brew update` before `brew upgrade --cask magent`. The release workflow pushes the updated cask to `vapor-pawelw/homebrew-tap`, but a user's local tap clone is only refreshed on demand. Without `brew update`, `brew upgrade --cask magent` compares against the stale cached cask version — which can be older than the currently installed app — and becomes a silent no-op. The app then relaunches at the same version, and the user sees "updating via Homebrew" followed by no visible change.
