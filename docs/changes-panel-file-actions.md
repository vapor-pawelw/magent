...
- `scrollSectionIntoViewIfNeeded` must call `section.scrollToVisible(section.bounds)` — NOT `scrollView.contentView.scrollToVisible(section.convert(bounds, to: sectionsStackView))`. `sectionsStackView` is non-flipped (Y=0 at bottom) while `contentView` is a `FlippedDiffClipView` (Y=0 at top). Passing sectionsStackView-space coords to the clip view's `scrollToVisible` inverts the Y axis: files lower in the visual list have smaller Y values in non-flipped space, so the clip view scrolls them progressively further up. Using `section.scrollToVisible(section.bounds)` lets AppKit perform the coordinate conversion correctly through the view hierarchy.
