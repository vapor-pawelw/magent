...
- `selectedCommitHash: String?` — `nil` = "Uncommitted" selected; non-nil = a commit hash. Reset to `nil` on each `update()` call (new thread selection).
...
- `rebuildCommitsRows()` — always adds the "Uncommitted" `CommitRowView` first, then commits, then "Load More".
