...
- **Commit detail mode must suppress the context badge**: the commit-detail header (with `‹ Back`) and the context badge share the same top row region. Keep `updateContextThreadIndicatorVisibility()` gated by `!isInCommitDetailMode`, and re-run visibility when exiting detail mode so the badge returns without overlapping the header.
