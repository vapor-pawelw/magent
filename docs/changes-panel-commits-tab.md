...
- **`outlineViewSelectionDidChange` passes `preserveSelection: !selectionChanged`**: when a new thread is selected (`selectionChanged = true`), the tab resets cleanly; when the same thread is re-selected programmatically after a reload, the tab is preserved.
