...
**Visual feedback:** `renameThreadFromPrompt` adds the thread to `autoRenameInProgress` before the AI call and removes it (via `defer`) when it exits. This drives the same sidebar pulse animation as the auto-rename path. Without this, the user sees nothing for up to 15 seconds while the agent call is in-flight.
