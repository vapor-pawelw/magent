...
- Multiline description rows should use a stable fixed row height and a stable description font (semibold) so selection-side effects (like unread state clearing) cannot rewrap rows between one and two lines.
