...
- **tmux layer** (`applyMouseWheelScrollSettings`): `magentDefaultScroll` binds `WheelUpPane`/`WheelDownPane` in the `root` table to always enter copy-mode (history-only, never passed to apps) and uses single-line `scroll-up`/`scroll-down` steps per wheel event (no `-N` multiplier). **Crucially, it also rebinds `WheelUpPane`/`WheelDownPane` in the `copy-mode` AND `copy-mode-vi` keytables to drop the default `-N5` multiplier.** tmux 3.6a's defaults for both copy-mode tables are `send -N5 -X scroll-up` — once a pane is in copy-mode (which happens after the very first wheel event that triggers `root`'s binding), every subsequent wheel event hits the copy-mode binding and gets silently multiplied by 5. Without overriding both tables, Magent's otherwise-correct `±5` cap + `mouse-scroll-multiplier = 1` combo still produces ~15 lines per notch. `allowAppsToCapture` unbinds `root` so tmux's default behavior applies (apps that request mouse get the events) and still strips the `-N5` from the copy-mode tables to keep history scrolling sane there. Both non-inherit options enable `set -g mouse on`. Changes are applied from `AppDelegate` via an async `Task`, **and re-applied after `tmux.killServer()` in `ThreadManager.restartTmuxAndRecoverSessions()`** because `bind-key` state is server-global and evaporates on server restart. The copy-mode bindings must be written as bare `tmux bind-key -T copy-mode WheelUpPane send-keys -X scroll-up` with no `select-pane \;` prefix — `ShellExecutor` runs via `/bin/sh -c`, and sh strips `\;` to a bare `;` which tmux's argv parser then treats as a command-list separator, splitting the bind body and leaving `WheelUpPane` bound to `select-pane` alone (no scroll) while `send-keys -X scroll-up` fires once at bind time. The resulting symptom is exactly-one-line-per-wheel until Esc exits copy-mode.
...
- **Discrete wheel normalization** (`TerminalSurfaceView.scrollWheel`): For non-precision devices (physical mouse wheels), normalize raw deltas to a fixed `±5` per notch before forwarding to Ghostty. Also treat no-phase wheel packets (`event.phase == []` and `event.momentumPhase == []`) as discrete even when AppKit marks them as "precise". Additionally, treat coarse integral packets (for example `±15` per notch) as discrete even when phases are present, because some mice still emit notch-style wheel data through that code path.
- **Ghostty `mouse-scroll-multiplier = 1`** (`GhosttyAppManager.writeOverrideConfig`): Ghostty's default `mouse-scroll-multiplier` is `3.0`, which silently inflates the number of mouse-wheel sequences forwarded to the embedded app — turning Magent's `±5` line cap into 15 tmux `WheelUpPane` events per notch. The embedded override config pins `mouse-scroll-multiplier = 1` so what Magent sends to `ghostty_surface_mouse_scroll` is exactly what tmux receives. Do not remove this without also lowering the per-event cap proportionally.
