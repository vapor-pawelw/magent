...
- **`reloadData()` transiently deselects and re-selects the current row**, which fires `outlineViewSelectionDidChange` with row = -1 then again with the restored thread. The row = -1 event called `diffPanelView?.clear()`, which hid the panel (height = 0) before the async git re-fetch could restore it — producing a visible blink. Fix: `ThreadListViewController.isReloadingData` is set for the duration of `reloadData()`; `outlineViewSelectionDidChange` skips `diffPanelView?.clear()` while this flag is set, so the panel stays visible through the transient deselection and only refreshes (without blanking) once the async fetch completes.
