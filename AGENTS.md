...
- **Initial prompt injection must guard against interactive shell blockers**: `injectAfterStart` waits up to 10 s for agent-ready signals via `waitForAgentReady`. If that times out, check the pane for interactive yes/no patterns (`[Y/n]`, `[y/N]`, `Press any key`, etc.) via `detectsInteractiveShellBlocker` before sending any text. If a blocker is found, abort injection and show a persistent warning banner with a Retry button; do **not** send the prompt into the wrong context. The crash-recovery file is intentionally left in place (no `magentAgentKeysInjected` posted) so the prompt survives an app restart.
